...
namespace Fargowiltas.Items.Summons.SwarmSummons.Thorium
{
    ...
    public class OverloadSaucer : ModItem
    {
        ...
        public override bool UseItem(Player player)
        {
            ...
            {
                int boss = NPC.NewNPC((int)player.position.X + Main.rand.Next(-1000, 1000), (int)player.position.Y + Main.rand.Next(-1000, -400), Fargowiltas.LoadedMods["Thorium"].NPCType("ThePrimeScouter"));
                Main.npc[boss].GetGlobalNPC<FargoGlobalNPC>().swarmActive = true;
            }

            if (Main.netMode == NetmodeID.Server)
            {
                ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Invaders have arrived!"), new Color(175, 75, 255));
            }
            else
            {
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(Fargowiltas.LoadedMods["Thorium"], "StarCaller");
